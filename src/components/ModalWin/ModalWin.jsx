import { useContext, useEffect } from 'react'
import {
    CardCategoryItemEducation,
    CardCategoryItemFood,
    CardCategoryItemHousing,
    CardCategoryItemJoy,
    CardCategoryItemOthers,
    CardCategoryItemTransport,
    ItemName,
    ModalBox,
} from './SModalWin.styled'
import { TransactionsContext } from '../../context/TransactionsContext'
import axios from 'axios'
import { AuthContext } from '../../context/AuthContext'

export const ModalWin = () => {
    const { filtredCategory, setFiltredCategory, setTransactions, fetchTransactions } =
        useContext(TransactionsContext)
        const {user} = useContext(AuthContext)
        const Token = user.user.token
        console.log(Token);
        
        
    async function filteredWithCategory() {
  try {
    const response = await axios.get(
      `https://wedev-api.sky.pro/api/transactions?filterBy=${filtredCategory}`,
      {
       
        headers: {
          Authorization: "Bearer " + Token,
          "Content-Type": "text/html",
        }
      }
    );
    console.log(response.data);
    
    setTransactions(response.data);
  } catch (error) {
    console.error('Ошибка фильтрации:', error);
    throw error; // Пробрасываем ошибку дальше
  }
}

useEffect(() => {
  if (filtredCategory) { 
    filteredWithCategory();
  } else {
    fetchTransactions()
  }
}, [filtredCategory])

    return (
        <ModalBox>
            <CardCategoryItemFood
                onClick={() =>
                    setFiltredCategory(
                        filtredCategory === 'food' ? null : 'food'
                    )
                }
                $isActive={filtredCategory}
            >
                <svg
                    width="14"
                    height="14"
                    viewBox="0 0 14 15"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <path
                        d="M11.2234 3.75507H10.99L9.01835 1.78341C8.86085 1.62591 8.60419 1.62591 8.44085 1.78341C8.28335 1.94091 8.28335 2.19757 8.44085 2.36091L9.83502 3.75507H4.16502L5.55919 2.36091C5.71669 2.20341 5.71669 1.94674 5.55919 1.78341C5.40169 1.62591 5.14502 1.62591 4.98169 1.78341L3.01585 3.75507H2.78252C2.25752 3.75507 1.16669 3.75507 1.16669 5.24841C1.16669 5.81424 1.28335 6.18757 1.52835 6.43257C1.66835 6.57841 1.83752 6.65424 2.01835 6.69507C2.18752 6.73591 2.36835 6.74174 2.54335 6.74174H11.4567C11.6375 6.74174 11.8067 6.73007 11.97 6.69507C12.46 6.57841 12.8334 6.22841 12.8334 5.24841C12.8334 3.75507 11.7425 3.75507 11.2234 3.75507Z"
                        fill="black"
                    />
                    <path
                        d="M11.1126 7.5H2.8409C2.47924 7.5 2.20507 7.82083 2.2634 8.17667L2.7534 11.175C2.91674 12.1783 3.35424 13.3333 5.29674 13.3333H8.56924C10.5351 13.3333 10.8851 12.3475 11.0951 11.245L11.6842 8.19417C11.7542 7.8325 11.4801 7.5 11.1126 7.5ZM6.18924 11.2625C6.18924 11.49 6.0084 11.6708 5.78674 11.6708C5.55924 11.6708 5.3784 11.49 5.3784 11.2625V9.3375C5.3784 9.11583 5.55924 8.92917 5.78674 8.92917C6.0084 8.92917 6.18924 9.11583 6.18924 9.3375V11.2625ZM8.6859 11.2625C8.6859 11.49 8.50507 11.6708 8.27757 11.6708C8.0559 11.6708 7.86924 11.49 7.86924 11.2625V9.3375C7.86924 9.11583 8.0559 8.92917 8.27757 8.92917C8.50507 8.92917 8.6859 9.11583 8.6859 9.3375V11.2625Z"
                        fill="black"
                    />
                </svg>
                <ItemName>Еда</ItemName>
            </CardCategoryItemFood>
            <CardCategoryItemTransport
                onClick={() =>
                    setFiltredCategory(
                        filtredCategory === 'transport' ? null : 'transport'
                    )
                }
                $isActive={filtredCategory}
            >
                <svg
                    width="14"
                    height="14"
                    viewBox="0 0 14 15"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <path
                        d="M12.6875 5.16671C12.6875 5.40588 12.4892 5.60421 12.25 5.60421H1.75C1.51083 5.60421 1.3125 5.40588 1.3125 5.16671C1.3125 4.92755 1.51083 4.72921 1.75 4.72921H2.345L2.56667 3.67338C2.77667 2.65255 3.21417 1.71338 4.9525 1.71338H9.0475C10.7858 1.71338 11.2233 2.65255 11.4333 3.67338L11.655 4.72921H12.25C12.4892 4.72921 12.6875 4.92755 12.6875 5.16671Z"
                        fill="black"
                    />
                    <path
                        d="M12.9383 8.46817C12.8508 7.50567 12.5941 6.479 10.7216 6.479H3.27828C1.40578 6.479 1.15495 7.50567 1.06162 8.46817L0.734948 12.0207C0.694115 12.464 0.839948 12.9073 1.14328 13.2398C1.45245 13.5782 1.88995 13.7707 2.35661 13.7707H3.45328C4.39828 13.7707 4.57911 13.2282 4.69578 12.8723L4.81245 12.5223C4.94661 12.1198 4.98161 12.0207 5.50661 12.0207H8.49328C9.01828 12.0207 9.03578 12.079 9.18745 12.5223L9.30411 12.8723C9.42078 13.2282 9.60161 13.7707 10.5466 13.7707H11.6433C12.1041 13.7707 12.5474 13.5782 12.8566 13.2398C13.1599 12.9073 13.3058 12.464 13.2649 12.0207L12.9383 8.46817ZM5.24995 9.68734H3.49995C3.26078 9.68734 3.06245 9.489 3.06245 9.24984C3.06245 9.01067 3.26078 8.81234 3.49995 8.81234H5.24995C5.48911 8.81234 5.68745 9.01067 5.68745 9.24984C5.68745 9.489 5.48911 9.68734 5.24995 9.68734ZM10.4999 9.68734H8.74995C8.51078 9.68734 8.31245 9.489 8.31245 9.24984C8.31245 9.01067 8.51078 8.81234 8.74995 8.81234H10.4999C10.7391 8.81234 10.9374 9.01067 10.9374 9.24984C10.9374 9.489 10.7391 9.68734 10.4999 9.68734Z"
                        fill="black"
                    />
                </svg>
                <ItemName>Транспорт</ItemName>
            </CardCategoryItemTransport>
            <CardCategoryItemHousing
                onClick={() => {
                    setFiltredCategory(
                        filtredCategory === 'housing' ? null : 'housing'
                    )
                }}
                $isActive={filtredCategory}
            >
                <svg
                    width="14"
                    height="14"
                    viewBox="0 0 14 15"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <path
                        d="M12.8334 12.8958H12.25V6.32167C12.25 5.96 12.0867 5.62167 11.8009 5.4L11.0834 4.84L11.0717 3.41084C11.0717 3.09 10.8092 2.83334 10.4884 2.83334H8.49919L7.71752 2.22667C7.29752 1.89417 6.70252 1.89417 6.28252 2.22667L2.19919 5.4C1.91335 5.62167 1.75002 5.96 1.75002 6.31584L1.72085 12.8958H1.16669C0.92752 12.8958 0.729187 13.0942 0.729187 13.3333C0.729187 13.5725 0.92752 13.7708 1.16669 13.7708H12.8334C13.0725 13.7708 13.2709 13.5725 13.2709 13.3333C13.2709 13.0942 13.0725 12.8958 12.8334 12.8958ZM3.79169 7.9375V7.0625C3.79169 6.74167 4.05419 6.47917 4.37502 6.47917H5.54169C5.86252 6.47917 6.12502 6.74167 6.12502 7.0625V7.9375C6.12502 8.25834 5.86252 8.52084 5.54169 8.52084H4.37502C4.05419 8.52084 3.79169 8.25834 3.79169 7.9375ZM8.45835 12.8958H5.54169V11.2917C5.54169 10.8075 5.93252 10.4167 6.41669 10.4167H7.58335C8.06752 10.4167 8.45835 10.8075 8.45835 11.2917V12.8958ZM10.2084 7.9375C10.2084 8.25834 9.94585 8.52084 9.62502 8.52084H8.45835C8.13752 8.52084 7.87502 8.25834 7.87502 7.9375V7.0625C7.87502 6.74167 8.13752 6.47917 8.45835 6.47917H9.62502C9.94585 6.47917 10.2084 6.74167 10.2084 7.0625V7.9375Z"
                        fill="black"
                    />
                </svg>
                <ItemName>Жилье</ItemName>
            </CardCategoryItemHousing>
            <CardCategoryItemJoy
                onClick={() =>
                    setFiltredCategory(filtredCategory === 'joy' ? null : 'joy')
                }
                $isActive={filtredCategory}
            >
                <svg
                    width="14"
                    height="14"
                    viewBox="0 0 14 15"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <path
                        d="M9.91667 1.6665H4.08333C2.8 1.6665 1.75 2.7165 1.75 3.99984V10.9998C1.75 12.2832 2.8 13.3332 4.08333 13.3332H9.91667C11.2 13.3332 12.25 12.2832 12.25 10.9998V3.99984C12.25 2.7165 11.2 1.6665 9.91667 1.6665ZM6.335 11.0815C6.2475 11.169 6.13667 11.2098 6.02583 11.2098C5.915 11.2098 5.80417 11.169 5.71667 11.0815L5.3375 10.7023L4.97583 11.064C4.88833 11.1515 4.7775 11.1923 4.66667 11.1923C4.55583 11.1923 4.445 11.1515 4.3575 11.064C4.18833 10.8948 4.18833 10.6148 4.3575 10.4457L4.71917 10.084L4.375 9.73984C4.20583 9.57067 4.20583 9.29067 4.375 9.1215C4.54417 8.95234 4.82417 8.95234 4.99333 9.1215L5.3375 9.46567L5.69917 9.104C5.86833 8.93484 6.14833 8.93484 6.3175 9.104C6.48667 9.27317 6.48667 9.55317 6.3175 9.72234L5.95583 10.084L6.335 10.4632C6.50417 10.6323 6.50417 10.9123 6.335 11.0815ZM8.4525 11.2857C8.13167 11.2857 7.86917 11.029 7.86917 10.7082V10.6965C7.86917 10.3757 8.13167 10.1132 8.4525 10.1132C8.77333 10.1132 9.03583 10.3757 9.03583 10.6965C9.03583 11.0173 8.77333 11.2857 8.4525 11.2857ZM9.63083 10.0257C9.31 10.0257 9.04167 9.76317 9.04167 9.44234C9.04167 9.1215 9.29833 8.859 9.61917 8.859H9.63083C9.95167 8.859 10.2142 9.1215 10.2142 9.44234C10.2142 9.76317 9.95167 10.0257 9.63083 10.0257ZM10.5 5.89567C10.5 6.45567 10.0392 6.9165 9.47917 6.9165H4.52083C3.96083 6.9165 3.5 6.45567 3.5 5.89567V4.43734C3.5 3.87734 3.96083 3.4165 4.52083 3.4165H9.47917C10.0392 3.4165 10.5 3.87734 10.5 4.43734V5.89567Z"
                        fill="black"
                    />
                </svg>
                <ItemName>Развлечения</ItemName>
            </CardCategoryItemJoy>
            <CardCategoryItemEducation
                onClick={() =>
                    setFiltredCategory(
                        filtredCategory === 'education' ? null : 'education'
                    )
                }
                $isActive={filtredCategory}
            >
                <svg
                    width="14"
                    height="14"
                    viewBox="0 0 14 15"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <path
                        d="M9.81755 9.62342C10.2084 9.36676 10.7217 9.64676 10.7217 10.1134V10.8659C10.7217 11.6068 10.1442 12.4001 9.45005 12.6334L7.58922 13.2518C7.26255 13.3626 6.73171 13.3626 6.41088 13.2518L4.55005 12.6334C3.85005 12.4001 3.27838 11.6068 3.27838 10.8659V10.1076C3.27838 9.64676 3.79171 9.36676 4.17671 9.61759L5.37838 10.3993C5.83922 10.7084 6.42255 10.8601 7.00588 10.8601C7.58922 10.8601 8.17255 10.7084 8.63338 10.3993L9.81755 9.62342Z"
                        fill="black"
                    />
                    <path
                        d="M11.655 4.26841L8.16084 1.97591C7.53084 1.56174 6.49251 1.56174 5.86251 1.97591L2.35084 4.26841C1.22501 4.99757 1.22501 6.64841 2.35084 7.38341L3.28418 7.99007L5.86251 9.67007C6.49251 10.0842 7.53084 10.0842 8.16084 9.67007L10.7217 7.99007L11.5208 7.46507V9.25007C11.5208 9.48924 11.7192 9.68757 11.9583 9.68757C12.1975 9.68757 12.3958 9.48924 12.3958 9.25007V6.38007C12.6292 5.62757 12.39 4.75257 11.655 4.26841Z"
                        fill="black"
                    />
                </svg>
                <ItemName>Образование</ItemName>
            </CardCategoryItemEducation>
            <CardCategoryItemOthers
                onClick={() =>
                    setFiltredCategory(
                        filtredCategory === 'others' ? null : 'others'
                    )
                }
                $isActive={filtredCategory}
            >
                <svg
                    width="14"
                    height="14"
                    viewBox="0 0 14 15"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <path
                        d="M9.33332 1.6665H4.66666C2.33332 1.6665 1.16666 2.83317 1.16666 5.1665V12.7498C1.16666 13.0707 1.42916 13.3332 1.74999 13.3332H9.33332C11.6667 13.3332 12.8333 12.1665 12.8333 9.83317V5.1665C12.8333 2.83317 11.6667 1.6665 9.33332 1.6665ZM8.16666 9.39567H4.08332C3.84416 9.39567 3.64582 9.19734 3.64582 8.95817C3.64582 8.719 3.84416 8.52067 4.08332 8.52067H8.16666C8.40582 8.52067 8.60416 8.719 8.60416 8.95817C8.60416 9.19734 8.40582 9.39567 8.16666 9.39567ZM9.91666 6.479H4.08332C3.84416 6.479 3.64582 6.28067 3.64582 6.0415C3.64582 5.80234 3.84416 5.604 4.08332 5.604H9.91666C10.1558 5.604 10.3542 5.80234 10.3542 6.0415C10.3542 6.28067 10.1558 6.479 9.91666 6.479Z"
                        fill="black"
                    />
                </svg>
                <ItemName>Другое</ItemName>
            </CardCategoryItemOthers>
        </ModalBox>
    )
}
